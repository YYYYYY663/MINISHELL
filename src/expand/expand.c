...
/*   expand.c                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: yukobaya <yukobaya@student.42tokyo.jp>     +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/28 12:28:16 by ymizukam          #+#    #+#             */
/*   Updated: 2025/02/15 23:41:22 by yukobaya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// シングルクォートを処理
static char *process_single_quote(const char **value, int *sin_q_flag)
{
    const char *start; 
    size_t len;
    char *result;
...
static char *process_no_quote(const char **value, t_info *info)
{
    ...
    ifs = get_ifs();
    result = xmalloc(INIT_BUFFER_SIZE, info);
    ptr = result;
...
static char *process_value(const char **value, t_info *info, int *sin_q_flag)
{
    if (**value == '\'')
        return process_single_quote(value, sin_q_flag);
    else if (**value == '"')
...
static char *apply_expansion(const char *value, t_info *info, int *sin_q_flag)
{
    size_t new_len;
    char *expanded; 
    char *result;
    ...
    expanded= xmalloc(new_len + 1, info);
    result = expanded;
    while (*value)
    {
        processed = process_value(&value, info, sin_q_flag);  
        if (processed) 
        {
...
void expand_dollar(t_list **lst, t_info *info, int *sin_q_flag)
{
    t_token *token;
    char *new_value;

    token = (t_token *)(*lst)->data;
    new_value = apply_expansion(token->value, info, sin_q_flag);
    if (!new_value)
    {
