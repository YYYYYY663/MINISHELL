...
static char	*expand_variable(const char **value, t_info *info)
{
	...
	var_len = 0;
	(*value)++; // '$' をスキップ
	while ((*value)[var_len] && (ft_isalnum((*value)[var_len])
			|| (*value)[var_len] == '_'))
	{
...
static char	*expand_and_copy(char *result, char **expanded, size_t *new_len,
...
static char	*split_and_copy_tokens(char *expanded, char *ptr, char **result,
		size_t *buffer_size)
{
	char	*start;
	char	*ifs;
...
static char	*handle_variable_expansion(const char **value, char *ptr,
